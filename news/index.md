...
- [`run_simspades()`](https://github.com/FOR-CAST/SpaDES.targets/reference/run_simspades.md)
  ...
  set, the run is invoked with a **scalar** `debug = 1` (which makes
  SpaDES emit the per-event trace + informational output as messages)
  and every message is captured to `log_file`. A scalar `debug` is
  deliberate: it dodges the debug-as-list crashes in current SpaDES.core
  – `debugToVerbose()` returning a per-element vector into
  `setPaths(silent = verbose <= 0)` (PredictiveEcology/SpaDES.core#322),
  and an unguarded `ifelse(debug < 1, ...)` in the event loop – both of
  which error “‘list’ object cannot be coerced”/“condition has length \>
  1” when `debug` is a list. Capturing messages ourselves also puts the
  trace in the file regardless of SpaDES.core’s own (currently broken)
  file-appender routing. (0.0.0.9016/.9017 briefly used a `debug`-list
  form, which crashed stages with events; 0.0.0.9018 switched to the
  scalar-`debug` + message-capture approach.) Because base
